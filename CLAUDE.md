...
| paste from… | `#btnPaste`, `parseExcel`, `excelTable`, `pasteExcel` | **Paste ▾** in the doc-bar opens a `showMenu` dropdown (only "From Excel" so far). `pasteExcel(keep)` reads `navigator.clipboard.readText()` (toasts like the Image button when blocked/denied/empty), `parseExcel` splits Excel's text (CRLF rows, tab cells, `"…"` around cells with line breaks/tabs, trailing CRLF dropped, ragged rows padded) and `excelTable` builds the same `<thead><th>` + `<tbody>` shape as the Table button (first row = header), followed by `<p><br></p>`. In view/source mode it enters edit mode first (caret at the end); while editing it restores the caret captured when the menu opened (`keep`), since clicking a menu item blurs the editor. Ends with `commit()`. |
