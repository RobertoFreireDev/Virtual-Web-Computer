...
```
virtualwebpc.html   the whole application (CSS + markup + JS, ~1800 lines)
...
| paste from… | `#tbPaste`, `parseExcel`, `excelTable`, `pasteExcel` | **Paste ▾** in the editing toolbar (right after **Image**, so only visible in edit mode) opens a `showMenu` dropdown (only "From table" so far). `pasteExcel(keep)` reads `navigator.clipboard.readText()` (toasts like the Image button when blocked/denied/empty), `parseExcel` splits Excel's text (CRLF rows, tab cells, `"…"` around cells with line breaks/tabs, trailing CRLF dropped, ragged rows padded) and `excelTable` builds the same `<thead><th>` + `<tbody>` shape as the Table button (first row = header), followed by `<p><br></p>`. It restores the caret captured when the menu opened (`keep`), since clicking a menu item blurs the editor, and inserts there. Ends with `commit()`. |
